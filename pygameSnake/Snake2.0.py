import pygame, time, random, operator
...
def print_highscores(highscores):
    y_displacementing = -180
    if len(highscores) > 0:
        for item in range(len(highscores)):
            if y_displacementing < 200:
                label(highscores[item][0] + " : " + str(highscores[item][1]) , black , y_displacementing , 'medium')
                y_displacementing += 40

########## HIGHSCORES ##########
def highscore(score):
    highscores = []
    highscores0 = [line.strip() for line in open("highscore.txt", 'r')]
    for i in range(len(highscores0)):
        i -= 1
        highscores.append([highscores0[i] , highscores0[i+1]])
    for item in range(len(highscores)):
        highscores[item][1] = int(highscores[item][1])
    highscores = list(reversed(sorted(highscores[1::2],key=itemgetter(1))))
    hs = True
    done = False
    new_name = ""
    if hs == True:
        gameDisplay.fill(white)
        label("HIGHSCORES" , black , -250 , 'large')
        label("Press [R] to restart     [Q] to quit     [RSHIFT] to enter your score" , black , 250 , 'small')
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_r:
                        ...
                        for event in pygame.event.get():
                            if event.type == KEYDOWN:
                                if event.key == K_RETURN:
                                    highscores.append([new_name , str((snake_length - 1))])
                                    highscores = list(reversed(sorted(highscores,key=itemgetter(1))))
                                    with open("highscore.txt", 'w') as file:
                                        for item in range(len(highscores)):
                                            file.write((highscores[item][0] + "\n"))
                                            file.write((str(highscores[item][1]) + "\n"))
                                    file.close()
                                    new_name = ""
                                    done = True
                                    ...
                                    new_name += str(event.unicode)
                                    gameDisplay.fill(white)
                                    label(new_name, black , 0 , 'small')
                                    pygame.display.update()
                    ...
                    gameDisplay.fill(white)
                    label("HIGHSCORES", black, -250, 'large')
                    label("Press [R] to restart     [Q] to quit", black, 250, 'small')
                    print_highscores(highscores)
                    pygame.display.update()
                    clock.tick(5)
                    for event in pygame.event.get():
...
def gameLoop():
    ...
    while not gameExit:
        ...
        if gameOver == True:
            ...
            label("Press [H] for HIGHSCORES" , black , 150 , "small")
            label("Score : " + str(snake_length - 1) , black , 80 , "medium")
            pygame.display.update()
